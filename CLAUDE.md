...
**Piste complémentaire activée le 18/07/2026 : export PDF programmé natif Semrush.** Semrush propose une fonctionnalité native (bouton "PDF" du dashboard Position Tracking) pour envoyer automatiquement un rapport PDF par email, sans passer par API ni navigateur automatisé. Configuré : envoi **hebdomadaire, chaque lundi, à `webmaster@mpitech.com`**, sujet "Semrush PDF Export". Contrairement au scraping ou à l'automatisation de connexion (options écartées le 18/07/2026 pour risque de blocage de compte et absence de coffre-fort à secrets côté cloud, voir discussion), **cette solution ne nécessite aucun identifiant stocké nulle part** — c'est Semrush lui-même qui envoie l'email, aucune connexion à automatiser.
- **Ce que ça résout** : la donnée arrive automatiquement dans une boîte mail chaque lundi, sans intervention.
- **Ce qui reste à construire** : rien ne lit encore cette boîte automatiquement pour en extraire les données dans `data/position-tracking-latest.json` à la place du rafraîchissement manuel. `webmaster@mpitech.com` est une boîte Microsoft/Outlook (MPI Tech utilise Exchange/Outlook, pas Gmail — confirmé par l'usage d'Outlook sur le poste de l'utilisateur), donc le connecteur claude.ai Gmail ne s'applique pas ici ; il faudrait soit un connecteur Outlook/Microsoft 365 (à chercher, éventuellement via Composio), soit continuer à relever cet email manuellement en complément du rafraîchissement par navigateur. Tant que ce n'est pas résolu, le processus manuel ci-dessus reste la voie principale — le PDF programmé est un filet de sécurité et une source de vérification croisée, pas (encore) un remplacement.
...
**Dernière vérification : 20/07/2026 (rapport hebdomadaire, semaine du 13/07 au 19/07/2026).** Toujours aucun nouvel article depuis le 13/07/2026 (reconfirmé, vérification directe de la page blog). Articles constatés sur le blog SEAL Systems, du plus récent au plus ancien (liste étendue jusqu'à mi-avril 2026 à l'occasion de la vérification du 18/07/2026) :
...
**Re-vérification du 20/07/2026** (rapport hebdomadaire, semaine du 13/07 au 19/07/2026) : toujours aucun nouvel article depuis le 13/07/2026, liste inchangée. Le rythme bimensuel observé (dernier article il y a 7 jours) rend un nouvel article plausible dans les jours qui viennent.
...
**GA4 et Search Console n'ont pas de connecteur natif dans le répertoire claude.ai** (recherche confirmée le 17/07/2026 : rien en cherchant "Google Analytics" ni "Search Console", seuls des agrégateurs tiers payants comme Supermetrics ou Windsor.ai ressortent). La voie retenue est de passer par **Composio**, une plateforme d'intégrations qui peut exposer des toolkits Google Analytics/Search Console une fois configurés côté Composio.

✅ **Confirmé le 19/07/2026** : dans le dashboard Composio (`dashboard.composio.dev`, workspace `webmaster_workspace`), les toolkits **Google Analytics** et **Google Search Console** sont bien **connectés et actifs** (onglet "Connected", chacun "1 Active"), déjà configurés lors d'une session antérieure ("Scopiom reporting setup"). Preuve d'usage réel : 68 appels d'outils déjà effectués sur ce compte, "Most Used App: Google Search Console". Donc les routines cloud ont bien accès aux vraies données GA4/Search Console via Composio — ce n'est plus une inconnue. Les rapports déjà générés (§10.5) s'appuyaient probablement déjà sur ces données réelles plutôt que sur une absence signalée. Chaque routine vérifie néanmoins les outils disponibles au démarrage et signale leur absence plutôt que d'inventer des données, au cas où la connexion expirerait.

**Piste Outlook abandonnée (19/07/2026)** : dans l'optique de lire automatiquement le PDF Position Tracking programmé par Semrush (§5.4) vers `webmaster@mpitech.com`, tentative de connexion du toolkit **Outlook** dans Composio (bouton "Connect" du catalogue d'apps). Échec après 3 tentatives (bouton bloqué sur "Connecting..." sans jamais ouvrir la fenêtre d'autorisation Microsoft). **Cause réelle identifiée par l'utilisateur, pas un bloqueur de popup** : la connexion OAuth à la messagerie mpitech.com (Microsoft 365) nécessite un **accès administrateur de l'organisation**, que l'utilisateur n'a pas. Cette voie est donc fermée sauf si un administrateur IT de MPI Tech accorde le consentement applicatif requis. Le PDF Semrush programmé (§5.4) continue d'arriver chaque lundi dans la boîte, mais reste à relever manuellement en complément du rafraîchissement du snapshot.
