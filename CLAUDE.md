...
**Tentative Outlook non aboutie (19/07/2026)** : dans l'optique de lire automatiquement le PDF Position Tracking programmé par Semrush (§5.4) vers `webmaster@mpitech.com`, tentative de connexion du toolkit **Outlook** dans Composio (bouton "Connect" disponible dans le catalogue d'apps). Échec après 3 tentatives : le bouton reste bloqué sur "Connecting..." sans jamais ouvrir la fenêtre d'autorisation Microsoft — vraisemblablement le bloqueur de popup de Chrome qui empêche l'ouverture déclenchée par un clic automatisé (les clics via l'extension Claude in Chrome ne sont pas toujours reconnus comme un vrai geste utilisateur). **À refaire en cliquant manuellement** (un vrai clic humain devrait passer le blocage) si on veut boucler la lecture automatique du PDF Position Tracking.
